...
def menu():
    # Cria título e instruções centralizadas horizontalmente
...
Feito por: Lucas
    ...
    # Agrupa o texto e o painel interno no painel externo
    conteudo_externo = Group(texto_externo, painel_interno_centro, creditos)

    # Painel externo com o conteúdo agrupado
...
#Cria o prompt (entrada do usuário)
def solicita_arquivo_txt():
   
    menu()
    while True:    
        pergunta= Prompt.ask("Digite a url do arquivo de texto (.txt)").strip()
        url = pergunta.replace('"','')
...
def nome_excel():
    while True:
        os.system('cls')
        menu()
        nome = Prompt.ask("Digite o nome da planilha a ser salva (não precisa escrever .xlxs)")
        if not nome:
...
def gerar_planilha(dados, nome_arquivo):
    ...
    fundo_verde = PatternFill(start_color="4CAF50", end_color="4CAF50", fill_type="solid")
    borda = Border(
    ...
    for linha_idx, registro in enumerate(dados, start=2):
        for col_idx, cabecalho in enumerate(cabecalhos, start=1):
            cell = ws.cell(row=linha_idx, column=col_idx, value=registro.get(cabecalho, ""))
            cell.alignment = alinhamento_centro
            cell.border = borda
